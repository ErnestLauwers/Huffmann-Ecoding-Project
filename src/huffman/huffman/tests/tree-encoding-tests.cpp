...
namespace
{
    ...
    void check_tree_encoding()
    {
        ...
        auto originalTree = data::Branch<Datum>(std::move(branch3), std::move(branch4));
        auto output = buffer.destination()->create_output_stream();
        auto input = buffer.source()->create_input_stream();
        encoding::huffman::encode_tree(originalTree, 8, *output);
        auto decodedTree = encoding::huffman::decode_tree(8, *input);
        CHECK(!areTreesEqual(originalTree, *decodedTree));
    }
}
